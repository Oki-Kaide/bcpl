/*
############ UNDER DEVELOPMENT ###################################
This is the ARM version of the MC dynamic code generation package.

This is based on mci386.b but work on it has only just started (March
2014).
...
@ C Linkage:
@   On entry rl =r14  is the return address
...
@   result in r0
   ...
   stmfd sp!,{r4-r11,lr}   on entry

   ldmfd sp!, {r4-r11,pc}  to return
...
The machine dependent low level (ARM) code is compiled using ia
...
// ARM machine operations (all 32-bit unless otherwise specified).
...
// On the ARM, all instructions are 32-bit
